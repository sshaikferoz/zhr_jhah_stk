sap.ui.define([
    ...
    return ControllerExtension.extend("com.jhah.zhrjhahsecstk.ext.controller.StickerControllerExtension", {
         ...
         *
         *   /StickerMaster?$filter=ExpireDate gt <today>
        ...
        _requestActiveStickerCount: function () {
            ...
            // A list binding issues no request of its own: _CollectionCache's
            // fetchValue only waits for reads that are already in flight, so
            // getHeaderContext().requestProperty("$count") on a fresh binding
            // resolves to undefined without ever going to the server. Reading
            // the first page is what triggers the request, and $count above
            // brings the server-side total back with it. $top is capped at the
            // limit — the exact total beyond it doesn't change the outcome.
            return oBinding.requestContexts(0, MAX_ACTIVE_STICKERS).then(function (aContexts) {
                var iCount = oBinding.getCount();
                // Falls back to the page length if the service ignored $count;
                // capped at MAX_ACTIVE_STICKERS, which still trips the limit.
                return typeof iCount === "number" ? iCount : aContexts.length;
            });
        },
        ...
        _checkActiveStickerLimit: function () {
            return this._requestActiveStickerCount().then(function (iCount) {
                // An unusable count must never block: "undefined < 2" is false,
                // which would otherwise turn a bad read into a hard stop.
                if (typeof iCount !== "number" || iCount < MAX_ACTIVE_STICKERS) {
                    return;
                }
